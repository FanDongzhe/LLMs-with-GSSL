...
def aug_drop_node_mine(input_fea, input_adj, drop_percent=0.2):
    # Note: assume input_adj is in CSR format
    input_fea = input_fea.squeeze(0)
    ...
    aug_input_fea = np.delete(input_fea, drop_node_list, axis=0)
    aug_input_adj = delete_row_csr(input_adj, drop_node_list)

    aug_input_fea = torch.unsqueeze(torch.from_numpy(aug_input_fea), 0)
    # aug_input_adj remains as a CSR matrix
